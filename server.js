...
io.sockets.on('connection', function (socket) {
     ...
     */
    socket.on('grantAudioPermission', (socketId, room) => {
        if (socket.id === roomAdmin) {
            io.to(socketId).emit('grantAudioPermission', socketId);
        }
    });

    socket.on('grantVideoPermission', (socketId, room) => {
        if (socket.id === roomAdmin) {
            io.to(socketId).emit('grantVideoPermission', socketId);
        }
    });
    ...
    socket.on('newUserJoined', (room, socketId) => {
        if (socket.id === roomAdmin) {
            io.to(socketId).emit('newUserJoined', room, socketId);
        }
    });
});