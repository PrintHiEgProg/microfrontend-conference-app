...
class Webrtc extends EventTarget {
    constructor(
        ...
        this.mediaPermissions = { audio: false, video: false }; // Default permissions
    ...
    joinRoom(room) {
        ...
        if (!room) {
            // Если комната не указана, создаем новую комнату
            room = uuidv4();
        }
    ...
    getLocalStream(audioConstraints, videoConstraints) {
        return navigator.mediaDevices
            .getUserMedia({
                audio: this.mediaPermissions.audio ? audioConstraints : false,
                video: this.mediaPermissions.video ? videoConstraints : false,
            })
            .then((stream) => {
    ...
    _connect(socketId) {
        if (typeof this._localStream !== 'undefined' && this.isReady) {
            ...
            if (this._localStream) {
                ...
                this.warn('Local stream is null');
            }
    ...
    _onSocketListeners() {
        ...
        this.socket.on('created', (room, socketId) => {
            ...
            this._isAdmin = true;
            this.mediaPermissions = { audio: true, video: true }; // Admin has full permissions
        ...
        this.socket.on('joined', (room, socketId) => {
            ...
            this.isReady = true;
            this._myId = socketId;
            this.mediaPermissions = { audio: false, video: false }; // Default permissions for participants

            this._emit('joinedRoom', { roomId: room });

            // Notify the admin that a new user has joined
            this.socket.emit('newUserJoined', room, socketId);
        });
            ...
            switch (message.type) {
                ...
                case 'offer': // got connection offer
                    ...
                    this.pcs[socketId].setRemoteDescription(
                        ...
                        this.error('Error setting remote description: ', error);
                    });
                    break;
                ...
                case 'candidate': // received candidate sdp
                    ...
                    this.pcs[socketId].addIceCandidate(candidate).catch((error) => {
        ...
        this.socket.on('newUserJoined', (room, socketId) => {
            if (this.isAdmin && this.room === room) {
                this._connect(socketId);
            }
        });
    }
    ...
    _handleIceCandidate(socketId, event) {
        ...
        if (event.candidate) {
            this.log('Sending ICE candidate: ', event.candidate);
            this._sendMessage(
                {
                    type: 'candidate',
    ...
    _makeOffer(socketId) {
        ...
        this.pcs[socketId].createOffer(
            (offer) => {
                ...
                this._setSendLocalDescription(socketId, offer);
            },
        ...
        this.pcs[socketId]
            ...
            .then(
                (answer) => {
                    ...
                    this._setSendLocalDescription(socketId, answer);
                },
    ...
    grantAudioPermission(socketId) {
        this.mediaPermissions[socketId] = { audio: true, video: this.mediaPermissions[socketId]?.video || false };
        this._sendMessage({ type: 'grantAudioPermission' }, socketId);
    }

    grantVideoPermission(socketId) {
        this.mediaPermissions[socketId] = { audio: this.mediaPermissions[socketId]?.audio || false, video: true };
